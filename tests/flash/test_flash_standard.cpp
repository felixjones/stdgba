...
void write_marker(sf::sector_span buf) {
    buf[0] = g_marker;
}
void read_marker(sf::const_sector_span buf) {
    g_marker = buf[0];
}
void write_s0(sf::sector_span buf) {
    buf[0] = std::byte{0xAA};
}
void write_s1(sf::sector_span buf) {
    buf[0] = std::byte{0x55};
}
void read_s0(sf::const_sector_span buf) {
    g_s0_val = buf[0];
}
void read_s1(sf::const_sector_span buf) {
    g_s1_val = buf[0];
}
void write_aa(sf::sector_span buf) {
    buf[0] = std::byte{0xAA};
}
void write_55(sf::sector_span buf) {
    buf[0] = std::byte{0x55};
}
void read_b0(sf::const_sector_span buf) {
    g_b0_val = buf[0];
}
void read_b1(sf::const_sector_span buf) {
    g_b1_val = buf[0];
}
...
int main() {
    ...
    if (chip.mfr == flash::manufacturer::atmel) return gba::test.finish();

    // Basic erase + write + read cycle
    {
        for (std::size_t i = 0; i < g_write_data.size(); ++i) g_write_data[i] = static_cast<std::byte>(i ^ 0xA5);
        g_read_data = {};

        constexpr auto cmds = sf::compile(sf::erase_sector(0), sf::write_sector(0, write_test_data),
        ...
        for (std::size_t i = 0; i < g_write_data.size(); ++i) gba::test.eq(g_read_data[i], g_write_data[i]);
    }

    // Multiple sectors in one sequence
    {
        constexpr auto cmds = sf::compile(sf::erase_sector(0), sf::write_sector(0, write_s0), sf::erase_sector(1),
                                          sf::write_sector(1, write_s1), sf::read_sector(0, read_s0),
                                          sf::read_sector(1, read_s1));

        gba::test.eq(static_cast<int>(cmds.execute()), static_cast<int>(flash::error::success));
        gba::test.eq(g_s0_val, std::byte{0xAA});
        gba::test.eq(g_s1_val, std::byte{0x55});
    }
    ...
    // erase_chip enables writes to any sector
    {
        ...
        constexpr auto cmds = sf::compile(sf::erase_chip(), sf::write_sector(3, write_marker),
                                          sf::read_sector(3, read_marker));

        gba::test.eq(static_cast<int>(cmds.execute()), static_cast<int>(flash::error::success));
        gba::test.eq(g_marker, std::byte{0xBE});
    }
    ...
    // Read-only sequence (no erase needed)
    {
        ...
        constexpr auto cmds = sf::compile(sf::read_sector(3, read_marker));
    ...
    if (chip.chip_size == flash::size::flash_128k) {
        constexpr auto write_cmds = sf::compile(sf::switch_bank(0), sf::erase_sector(15),
                                                sf::write_sector(15, write_aa), sf::switch_bank(1),
                                                sf::erase_sector(15), sf::write_sector(15, write_55));

        auto err = write_cmds.execute();
        gba::test.eq(static_cast<int>(err), static_cast<int>(flash::error::success));

        constexpr auto read_cmds = sf::compile(sf::switch_bank(0), sf::read_sector(15, read_b0), sf::switch_bank(1),
    ...
    // Compile-time validation via bits::validate()
    {
        ...
        {
            constexpr sf::cmd seq[] = {sf::erase_chip(), sf::write_sector(5, write_marker),
                                       sf::write_sector(10, write_marker)};
        ...
        {
            constexpr sf::cmd seq[] = {sf::erase_sector(0), sf::write_sector(0, write_marker), sf::erase_sector(0),
                                       sf::write_sector(0, write_marker)};
    ...
    // Full sector write + verify (all 4096 bytes)
    {
        ...
        constexpr auto cmds = sf::compile(sf::erase_sector(2), sf::write_sector(2, write_full),
                                          sf::read_sector(2, read_full));

        gba::test.eq(static_cast<int>(cmds.execute()), static_cast<int>(flash::error::success));

        for (std::size_t i = 0; i < flash::sector_size; ++i) gba::test.eq(g_full_read[i], g_full_write[i]);
    }
    ...
    // Runtime placeholder -- erase + write + read with arg(0) as sector
    {
        ...
        constexpr auto cmds = sf::compile(sf::erase_sector(sf::arg(0)), sf::write_sector(sf::arg(0), write_marker),
    ...
    if (chip.chip_size == flash::size::flash_128k) {
        constexpr auto save = sf::compile(sf::switch_bank(sf::arg(0)), sf::erase_sector(sf::arg(1)),
        ...
        gba::test.eq(static_cast<int>(save.execute(0, 5)), static_cast<int>(flash::error::success));

        // Write 0x55 to bank 1 sector 5
        g_marker = std::byte{0x55};
        gba::test.eq(static_cast<int>(save.execute(1, 5)), static_cast<int>(flash::error::success));

        // Read back both
        constexpr auto load = sf::compile(sf::switch_bank(sf::arg(0)), sf::read_sector(sf::arg(1), read_marker));

        static_assert(load.num_args == 2);

        g_marker = std::byte{0};
        gba::test.eq(static_cast<int>(load.execute(0, 5)), static_cast<int>(flash::error::success));
        gba::test.eq(g_marker, std::byte{0xAA});

        g_marker = std::byte{0};
        gba::test.eq(static_cast<int>(load.execute(1, 5)), static_cast<int>(flash::error::success));
    ...
    // Runtime placeholder -- mixed static and arg commands
    {
        ...
        constexpr auto cmds = sf::compile(sf::erase_chip(), sf::write_sector(sf::arg(0), write_marker),
                                          sf::read_sector(sf::arg(0), read_marker));

        static_assert(cmds.num_args == 1);

        gba::test.eq(static_cast<int>(cmds.execute(7)), static_cast<int>(flash::error::success));
    ...
    // Compile-time validation with placeholders
    {
        ...
        {
            constexpr sf::cmd seq[] = {sf::switch_bank(sf::arg(0)), sf::erase_sector(sf::arg(1)),
                                       sf::write_sector(sf::arg(1), write_marker)};
            static_assert(validate(seq, 3));
        }
