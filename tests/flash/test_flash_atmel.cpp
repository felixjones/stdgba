...
void write_marker(af::page_span buf) { buf[0] = g_marker; }
void read_marker(af::const_page_span buf) { g_marker = buf[0]; }
void write_p0(af::page_span buf) { buf[0] = std::byte{0xAA}; }
void write_p1(af::page_span buf) { buf[0] = std::byte{0x55}; }
void read_p0(af::const_page_span buf) { g_p0_val = buf[0]; }
void read_p1(af::const_page_span buf) { g_p1_val = buf[0]; }
void write_aa(af::page_span buf) { buf[0] = std::byte{0xAA}; }
void write_55(af::page_span buf) { buf[0] = std::byte{0x55}; }
void read_b0(af::const_page_span buf) { g_b0_val = buf[0]; }
void read_b1(af::const_page_span buf) { g_b1_val = buf[0]; }
...
int main() {
    ...
    // compile() accepts write_page, read_page, switch_bank
    {
        constexpr auto cmds = af::compile(
            ...
            af::read_page(0, read_marker)
        );
        ASSERT_EQ(cmds.count, 2u);
    }
    ...
    // compile() with switch_bank
    {
        constexpr auto cmds = af::compile(
            af::switch_bank(0),
            af::write_page(100, write_marker),
            af::switch_bank(1),
            af::read_page(200, read_marker)
        );
        ASSERT_EQ(cmds.count, 4u);
    }
    ...
    // Page index range: 0 to pages_per_bank-1 (0-511)
    {
        constexpr auto cmds = af::compile(
            af::write_page(0, write_marker),
            af::write_page(511, write_marker)
        );
        ASSERT_EQ(cmds.count, 2u);
    }
    ...
    {
        constexpr auto cmds = af::compile(
            af::write_page(5, write_marker),
            af::read_page(10, read_marker),
            af::switch_bank(1)
        );
    ...
    if (chip.mfr == flash::manufacturer::atmel) {
        ...
        {
            for (std::size_t i = 0; i < g_write_page.size(); ++i)
                g_write_page[i] = static_cast<std::byte>(i ^ 0x5A);
            g_read_page = {};

            constexpr auto cmds = af::compile(
                af::write_page(0, write_page_data),
                af::read_page(0, read_page_data)
            );
            ...
            for (std::size_t i = 0; i < g_write_page.size(); ++i)
                ASSERT_EQ(g_read_page[i], g_write_page[i]);
        }
        ...
        // Multiple pages in one sequence
        {
            constexpr auto cmds = af::compile(
                af::write_page(0, write_p0),
                af::write_page(1, write_p1),
                af::read_page(0, read_p0),
                af::read_page(1, read_p1)
            );

            ASSERT_EQ(static_cast<int>(cmds.execute()), static_cast<int>(flash::error::success));
            ASSERT_EQ(g_p0_val, std::byte{0xAA});
            ASSERT_EQ(g_p1_val, std::byte{0x55});
        }
        ...
        // operator() as alias for execute()
        {
            ...
            constexpr auto cmds = af::compile(
                af::write_page(2, write_marker),
                af::read_page(2, read_marker)
            );

            ASSERT_EQ(static_cast<int>(cmds()), static_cast<int>(flash::error::success));
            ASSERT_EQ(g_marker, std::byte{0xEF});
        }

        // Bank switching (128KB chips only)
        if (chip.chip_size == flash::size::flash_128k) {
            constexpr auto write_cmds = af::compile(
                af::switch_bank(0),
                af::write_page(511, write_aa),
                af::switch_bank(1),
                af::write_page(511, write_55)
            );

            auto err = write_cmds.execute();
            ASSERT_EQ(static_cast<int>(err), static_cast<int>(flash::error::success));

            constexpr auto read_cmds = af::compile(
                af::switch_bank(0),
                af::read_page(511, read_b0),
                af::switch_bank(1),
    ...
    {
        constexpr auto cmds = af::compile(
            af::write_page(af::arg(0), write_marker),
    ...
    // Two args: bank + page
    {
        constexpr auto cmds = af::compile(
            ...
            af::write_page(af::arg(1), write_marker)
        );
    ...
    if (chip.mfr == flash::manufacturer::atmel) {
        ...
        if (chip.chip_size == flash::size::flash_128k) {
            ...
            ASSERT_EQ(static_cast<int>(save(0, 100)), static_cast<int>(flash::error::success));

            g_marker = std::byte{0x55};
            ASSERT_EQ(static_cast<int>(save(1, 100)), static_cast<int>(flash::error::success));

            constexpr auto load = af::compile(
                af::switch_bank(af::arg(0)),
                af::read_page(af::arg(1), read_marker)
            );

            g_marker = std::byte{0};
            ASSERT_EQ(static_cast<int>(load(0, 100)), static_cast<int>(flash::error::success));
            ASSERT_EQ(g_marker, std::byte{0xAA});
        ...
        // operator() with args
        {
            ...
            ASSERT_EQ(static_cast<int>(cmds(10)), static_cast<int>(flash::error::success));
