...
int main() {
    ...
    for (int off = 0; off < 4; ++off) {
        buf[off] = '\0';
        gba::test.eq(strlen_fn(&buf[off]), static_cast<std::size_t>(0));
    }
    ...
    for (int off = 0; off < 4; ++off) {
        buf[off] = 'X';
        buf[off + 1] = '\0';
        gba::test.eq(strlen_fn(&buf[off]), static_cast<std::size_t>(1));
    }
    ...
    for (int off = 0; off < 4; ++off) {
        for (int len = 1; len <= 7; ++len) {
            ...
            buf[off + len] = '\0';
            gba::test.eq(strlen_fn(&buf[off]), static_cast<std::size_t>(len));
        }
    }
    ...
    for (int pos = 0; pos < 4; ++pos) {
        ...
        buf[pos] = '\0';
        gba::test.eq(strlen_fn(buf), static_cast<std::size_t>(pos));
    }

    // Medium strings (8-32 bytes) at alignment 0
    for (int len = 8; len <= 32; ++len) {
        fill_pattern(len);
        gba::test.eq(strlen_fn(buf), static_cast<std::size_t>(len));
    }
    ...
    for (int wordpos = 0; wordpos < 4; ++wordpos) {
        fill_pattern(100);
        buf[60 + wordpos] = '\0';
        gba::test.eq(strlen_fn(buf), static_cast<std::size_t>(60 + wordpos));
    }
    ...
    // maxlen > strlen
    gba::test.eq(strnlen_fn(buf, 200), static_cast<std::size_t>(100));

    // maxlen == strlen
    gba::test.eq(strnlen_fn(buf, 100), static_cast<std::size_t>(100));

    // maxlen < strlen
    gba::test.eq(strnlen_fn(buf, 50), static_cast<std::size_t>(50));
    gba::test.eq(strnlen_fn(buf, 1), static_cast<std::size_t>(1));

    // maxlen == 0
    gba::test.eq(strnlen_fn(buf, 0), static_cast<std::size_t>(0));

    // strnlen with empty string
    buf[0] = '\0';
    gba::test.eq(strnlen_fn(buf, 100), static_cast<std::size_t>(0));
    gba::test.eq(strnlen_fn(buf, 0), static_cast<std::size_t>(0));
    ...
    for (int off = 0; off < 4; ++off) {
        for (int len = 1; len <= 16; ++len) {
            ...
            buf[off + len] = '\0';
            // maxlen > len
            gba::test.eq(strnlen_fn(&buf[off], len + 10), static_cast<std::size_t>(len));
            // maxlen == len
            gba::test.eq(strnlen_fn(&buf[off], len), static_cast<std::size_t>(len));
            // maxlen < len (when len > 1)
            if (len > 1) {
                gba::test.eq(strnlen_fn(&buf[off], len - 1), static_cast<std::size_t>(len - 1));
            }
        }
    }
