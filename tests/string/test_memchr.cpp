...
int main() {
    // count == 0: always NULL
    fill(0xAA, 16);
    ASSERT_EQ(addr(memchr_fn(buf, 0xAA, 0)), addr(nullptr));

    // Found at position 0
    fill(0xBB, 16);
    buf[0] = 0x42;
    ASSERT_EQ(addr(memchr_fn(buf, 0x42, 16)), addr(&buf[0]));

    // Found at various positions (word boundary testing)
    for (int pos = 0; pos < 32; ++pos) {
        fill(0xFF, 64);
        buf[pos] = 0x42;
        ASSERT_EQ(addr(memchr_fn(buf, 0x42, 64)), addr(&buf[pos]));
    }
    ...
    for (int off = 0; off < 4; ++off) {
        for (int pos = 0; pos < 20; ++pos) {
            fill(0xCC, 64);
            buf[off + pos] = 0x42;
            ASSERT_EQ(addr(memchr_fn(&buf[off], 0x42, 40)), addr(&buf[off + pos]));
    ...
    // Not found
    fill(0xAA, 64);
    ASSERT_EQ(addr(memchr_fn(buf, 0x42, 64)), addr(nullptr));

    // Not found (byte exists beyond count)
    fill(0xAA, 64);
    buf[32] = 0x42;
    ASSERT_EQ(addr(memchr_fn(buf, 0x42, 32)), addr(nullptr));
    ASSERT_EQ(addr(memchr_fn(buf, 0x42, 33)), addr(&buf[32]));

    // Search for 0x00
    fill(0xFF, 64);
    buf[10] = 0x00;
    ASSERT_EQ(addr(memchr_fn(buf, 0, 64)), addr(&buf[10]));

    // Long buffer
    fill(0xDD, 500);
    buf[499] = 0x42;
    ASSERT_EQ(addr(memchr_fn(buf, 0x42, 500)), addr(&buf[499]));
    ...
    buf[10] = 0x42;
    buf[20] = 0x42;
    ASSERT_EQ(addr(memchr_fn(buf, 0x42, 64)), addr(&buf[5]));

    // byte value > 127 (high bit set)
    fill(0x00, 64);
    buf[7] = 0x80;
    ASSERT_EQ(addr(memchr_fn(buf, 0x80, 64)), addr(&buf[7]));

    fill(0x00, 64);
    buf[7] = 0xFF;
    ASSERT_EQ(addr(memchr_fn(buf, 0xFF, 64)), addr(&buf[7]));
