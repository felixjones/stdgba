...
int main() {
    // Both empty
    s1[0] = s2[0] = '\0';
    gba::test.eq(strcmp_fn(s1, s2), 0);

    // One empty
    s1[0] = 'A';
    s1[1] = '\0';
    s2[0] = '\0';
    gba::test.eq(sign(strcmp_fn(s1, s2)), 1);
    gba::test.eq(sign(strcmp_fn(s2, s1)), -1);
    ...
    s1[21] = s2[22] = '\0';
    gba::test.eq(strcmp_fn(s1 + 1, s2 + 2), 0); // same content, different alignment

    // Misaligned and differing
    s2[12] = s1[11] + 1; // position 10 in both strings
    gba::test.eq(sign(strcmp_fn(s1 + 1, s2 + 2)), -1);

    // strncmp tests
    fill_same(20);

    // n == 0: always equal
    gba::test.eq(strncmp_fn(s1, s2, 0), 0);

    // n < strlen, strings equal within n
    gba::test.eq(strncmp_fn(s1, s2, 10), 0);

    // n == strlen
    gba::test.eq(strncmp_fn(s1, s2, 20), 0);

    // n > strlen
    gba::test.eq(strncmp_fn(s1, s2, 100), 0);

    // Differ at position 10, n > 10
    fill_same(20);
    s2[10] = s1[10] + 1;
    gba::test.eq(sign(strncmp_fn(s1, s2, 20)), -1);

    // Differ at position 10, n == 10 (before diff)
    gba::test.eq(strncmp_fn(s1, s2, 10), 0);

    // Differ at position 10, n == 11 (includes diff)
    gba::test.eq(sign(strncmp_fn(s1, s2, 11)), -1);

    // strncmp with n == 1
    fill_same(20);
    gba::test.eq(strncmp_fn(s1, s2, 1), 0);
    s2[0] = s1[0] + 1;
    gba::test.eq(sign(strncmp_fn(s1, s2, 1)), -1);

    // strncmp various lengths
    for (int len = 1; len <= 64; ++len) {
        fill_same(len);
        gba::test.eq(strncmp_fn(s1, s2, len), 0);
        gba::test.eq(strncmp_fn(s1, s2, len + 1), 0);
    }
