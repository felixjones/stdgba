...
void do_lmul_sg(long long a, long long b) { sink64 = __aeabi_lmul(a, b); }
void do_lmul_ab(long long a, long long b) { sink64 = bench_agbabi_lmul(a, b); }

void do_llsl_sg(long long v, int n) { sink64 = __aeabi_llsl(v, n); }
void do_llsl_ab(long long v, int n) { sink64 = bench_agbabi_llsl(v, n); }

void do_llsr_sg(long long v, int n) { sink64 = __aeabi_llsr(v, n); }
void do_llsr_ab(long long v, int n) { sink64 = bench_agbabi_llsr(v, n); }

void do_lasr_sg(long long v, int n) { sink64 = __aeabi_lasr(v, n); }
void do_lasr_ab(long long v, int n) { sink64 = bench_agbabi_lasr(v, n); }
...
int main() {
    test::with_logger([] {
        mgba_printf(MGBA_LOG_INFO, "=== lmul/shift benchmark (cycles) ===");
    });
    ...
    // 64-bit multiplication
    {
        ...
        test::with_logger([] {
            mgba_printf(MGBA_LOG_INFO, "  %-24s  stdgba   agbabi  save%%", "Case");
        });
    ...
    {
        // lmul
        ASSERT_EQ(__aeabi_lmul(2, 3), 6LL);
        ASSERT_EQ(__aeabi_lmul(0, -1LL), 0LL);
        ASSERT_EQ(__aeabi_lmul(0x100000000LL, 0x100000000LL), 0LL); // overflow
        ASSERT_EQ(__aeabi_lmul(0xFFFFFFFFLL, 0xFFFFFFFFLL), 0xFFFFFFFE00000001LL);
        ASSERT_EQ(__aeabi_lmul(-1LL, 2LL), -2LL);
        ASSERT_EQ(__aeabi_lmul(0x12345678LL, 1LL), 0x12345678LL);

        // llsl
        ASSERT_EQ(__aeabi_llsl(1, 0), 1LL);
        ASSERT_EQ(__aeabi_llsl(1, 1), 2LL);
        ASSERT_EQ(__aeabi_llsl(1, 31), 0x80000000LL);
        ASSERT_EQ(__aeabi_llsl(1, 32), 0x100000000LL);
        ASSERT_EQ(__aeabi_llsl(1, 63), static_cast<long long>(0x8000000000000000ULL));
        ASSERT_EQ(__aeabi_llsl(0xCAFEBABELL, 0), 0xCAFEBABELL);

        // llsr
        ASSERT_EQ(__aeabi_llsr(0x100000000LL, 32), 1LL);
        ASSERT_EQ(__aeabi_llsr(static_cast<long long>(0x8000000000000000ULL), 63), 1LL);
        ASSERT_EQ(__aeabi_llsr(0xFFLL, 4), 0xFLL);
        ASSERT_EQ(__aeabi_llsr(-1LL, 32), 0xFFFFFFFFLL);

        // lasr
        ASSERT_EQ(__aeabi_lasr(-1LL, 1), -1LL);
        ASSERT_EQ(__aeabi_lasr(-1LL, 32), -1LL);
        ASSERT_EQ(__aeabi_lasr(-1LL, 63), -1LL);
        ASSERT_EQ(__aeabi_lasr(-256LL, 4), -16LL);
        ASSERT_EQ(__aeabi_lasr(0x7FFFFFFFFFFFFFFFLL, 32), 0x7FFFFFFFLL);
    }
