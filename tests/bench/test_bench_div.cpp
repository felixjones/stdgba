...
void do_udiv_sg(unsigned int a, unsigned int b) { sink32 = __aeabi_uidiv(a, b); }
void do_udiv_ab(unsigned int a, unsigned int b) { sink32 = bench_agbabi_uidiv(a, b); }
void do_idiv_sg(int a, int b) { sink32 = static_cast<unsigned int>(__aeabi_idiv(a, b)); }
void do_idiv_ab(int a, int b) { sink32 = static_cast<unsigned int>(bench_agbabi_idiv(a, b)); }

// For 64-bit div, GCC's calling convention passes 64-bit args in r0:r1 and r2:r3
void do_uldiv_sg(unsigned long long a, unsigned long long b) { sink64 = __aeabi_uldivmod(a, b); }
void do_uldiv_ab(unsigned long long a, unsigned long long b) { sink64 = bench_agbabi_uldivmod(a, b); }
void do_ldiv_sg(long long a, long long b) { sink64 = __aeabi_ldivmod(a, b); }
void do_ldiv_ab(long long a, long long b) { sink64 = bench_agbabi_ldivmod(a, b); }
...
int main() {
    test::with_logger([] {
        mgba_printf(MGBA_LOG_INFO, "=== div/ldiv benchmark (cycles) ===");
    });
    ...
    // 32-bit signed division
    {
        ...
        static const struct { const char* name; int num; int denom; } cases[] = {
            { "100 / 10",              100, 10 },
            { "-100 / 10",            -100, 10 },
            { "100 / -10",             100, -10 },
            { "-100 / -10",           -100, -10 },
    ...
    // 64-bit signed division
    {
        ...
        static const struct { const char* name; long long num; long long denom; } cases[] = {
    ...
    ASSERT_EQ(__aeabi_uidiv(100, 10), 10u);
    ASSERT_EQ(__aeabi_idiv(-100, 10), -10);
    ASSERT_EQ(__aeabi_idiv(100, -10), -10);
