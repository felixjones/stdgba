...
void bench_backward(const char* title,
                    unsigned char* buf, std::size_t max_size,
                    unsigned int overlap,
                    const unsigned int* sizes, std::size_t n_sizes) {
    bench::print_header(title);
    test::with_logger([&] {
        mgba_printf(MGBA_LOG_INFO, "  %-8s  %6s %6s  %6s %6s  %5s",
            "Size", "stdgba", "stdg4", "agbabi", "agb4", "save%");
    });
    ...
    for (std::size_t si = 0; si < n_sizes; ++si) {
        ...
        const auto sg  = bench::measure_avg(iters, sg_fn,
            buf + overlap, buf, size);
        fill_buf(buf, size + overlap);
        const auto sg4 = bench::measure_avg(iters, sg4_fn,
            buf + overlap, buf, size);
        fill_buf(buf, size + overlap);
        const auto ab  = bench::measure_avg(iters, ab_fn,
            buf + overlap, buf, size);
        fill_buf(buf, size + overlap);
        const auto ab4 = bench::measure_avg(iters, ab4_fn,
            buf + overlap, buf, size);
...
int main() {
    test::with_logger([] {
        mgba_printf(MGBA_LOG_INFO,
            "=== memmove benchmark (backward copy, word-aligned) ===");
    });
    ...
    bench_backward("--- IWRAM backward (overlap=4) ---",
        iwram_buf, sizeof(iwram_buf), 4,
        iwram_sizes, std::size(iwram_sizes));

    // Overlap by 32 bytes: partial buffer shift
    bench_backward("--- IWRAM backward (overlap=32) ---",
        iwram_buf, sizeof(iwram_buf), 32,
        iwram_sizes, std::size(iwram_sizes));
    ...
    bench_backward("--- EWRAM backward (overlap=4) ---",
        ewram_buf, sizeof(ewram_buf), 4,
