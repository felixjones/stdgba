...
int main() {
    // 8x8 rect: fill entire sprite
    {
        ...
        for (int y = 0; y < 8; ++y)
            for (int x = 0; x < 8; ++x)
                ASSERT_EQ(read_pixel(s.data(), 8, x, y), 1);
    }
    ...
    // 8x8 empty: all transparent
    {
        ...
        for (int y = 0; y < 8; ++y)
            for (int x = 0; x < 8; ++x)
                ASSERT_EQ(read_pixel(s.data(), 8, x, y), 0);
    }
    ...
    // 8x8 partial rect: check inside and outside
    {
        ...
        ASSERT_EQ(read_pixel(s.data(), 8, 3, 3), 1);
        // Outside rect should be 0
        ASSERT_EQ(read_pixel(s.data(), 8, 0, 0), 0);
        ASSERT_EQ(read_pixel(s.data(), 8, 7, 7), 0);
    }

    // 16x16 sprite: multiple groups get different palette indices
    {
        constexpr auto s = sprite_16x16(
            rect(0, 0, 8, 8),      // group 1, palette_idx = 1
            rect(8, 0, 8, 8)       // group 2, palette_idx = 2
        );
        static_assert(s.size() == 128);
        static_assert(s.tile_count == 4);

        ASSERT_EQ(read_pixel(s.data(), 16, 2, 2), 1); // in group 1
        ASSERT_EQ(read_pixel(s.data(), 16, 10, 2), 2); // in group 2
    }
    ...
    // palette_idx override
    {
        ...
        ASSERT_EQ(read_pixel(s.data(), 8, 0, 0), 5);
    }

    // palette_idx(0) erases
    {
        constexpr auto s = sprite_8x8(
            rect(0, 0, 8, 8),     // palette_idx 1
            palette_idx(0),
            rect(2, 2, 4, 4)     // erase inner rect
        );
        // Inside erased area should be 0
        ASSERT_EQ(read_pixel(s.data(), 8, 3, 3), 0);
        // Outside erased area still palette 1
        ASSERT_EQ(read_pixel(s.data(), 8, 0, 0), 1);
    }

    // group: empty reserves palette index
    {
        constexpr auto s = sprite_8x8(
            group(),                 // palette_idx 1 (reserved, empty)
            rect(0, 0, 8, 8)       // palette_idx 2
        );
        ASSERT_EQ(read_pixel(s.data(), 8, 0, 0), 2);
    }
    ...
    // obj() returns correct attributes for 8x8
    {
        ...
        ASSERT_EQ(static_cast<unsigned>(obj.shape) & 3, static_cast<unsigned>(gba::shape_square));
        ASSERT_EQ(static_cast<unsigned>(obj.depth) & 1, static_cast<unsigned>(gba::depth_4bpp));
    }
    ...
    // Tall sprite (16x32)
    {
        ...
        static_assert(s.tile_count == 8);
        constexpr auto obj = s.obj();
        ASSERT_EQ(static_cast<unsigned>(obj.shape) & 3, static_cast<unsigned>(gba::shape_tall));
    }
    ...
    // circle
    {
        ...
        ASSERT_EQ(read_pixel(s.data(), 16, 8, 8), 1);
        // Far corners should be empty
        ASSERT_EQ(read_pixel(s.data(), 16, 0, 0), 0);
        ASSERT_EQ(read_pixel(s.data(), 16, 15, 15), 0);
    }
    ...
    // triangle
    {
        ...
        ASSERT_EQ(read_pixel(s.data(), 16, 8, 14), 1);
        // Top corners should be outside
        ASSERT_EQ(read_pixel(s.data(), 16, 0, 0), 0);
        ASSERT_EQ(read_pixel(s.data(), 16, 15, 0), 0);
    }
    ...
    // rect_outline
    {
        ...
        ASSERT_EQ(read_pixel(s.data(), 16, 2, 2), 1);
        ASSERT_EQ(read_pixel(s.data(), 16, 13, 13), 1);
        // Interior should be empty
        ASSERT_EQ(read_pixel(s.data(), 16, 7, 7), 0);
    ...
    // line
    {
        ...
        ASSERT_EQ(read_pixel(s.data(), 8, 0, 0), 1);
        ASSERT_EQ(read_pixel(s.data(), 8, 3, 3), 1);
        ASSERT_EQ(read_pixel(s.data(), 8, 7, 7), 1);
    }
    ...
    // oval_outline
    {
        ...
        // Edge should be filled
        ASSERT_NZ(read_pixel(s.data(), 16, 8, 2));
        // Center should be empty
        ASSERT_EQ(read_pixel(s.data(), 16, 8, 8), 0);
    }
}
