...
| `0x4000128` | `reg_siocnt_uart`  | RW     | `sio_uart_control`      | `REG_SIOCNT`         |
...
The serial registers at `0x4000120`-`0x400012A` are aliased for different modes. Use `reg_siocnt` for Normal mode,
`reg_siocnt_multi` for Multi-Player mode, and `reg_siocnt_uart` for UART mode. In UART mode, `reg_siodata8` addresses
the send/receive shift register, or the send/receive FIFO when enabled. Likewise `reg_siodata32` / `reg_siomulti` share
the same address.
