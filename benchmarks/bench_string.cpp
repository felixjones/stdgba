...
int main() {
    bench::with_logger(
        [&] { bench::log_printf(gba::log::level::info, "=== string function benchmarks (ROM/Thumb) ==="); });

    // strlen benchmark
    bench::print_header("--- strlen (cycles) ---");
    bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  Length     cycles"); });
    ...
    for (int len : {0, 1, 3, 4, 7, 8, 15, 16, 31, 32, 63, 64, 128, 256, 512, 1024, 4096}) {
        ...
        bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  %5d    %6u", len, cycles); });
    }

    // strlen at various alignments
    bench::print_header("--- strlen alignment (len=64) ---");
    bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  Offset    cycles"); });
    for (int off = 0; off < 4; ++off) {
        fill_string(str1 + off, 64);
        unsigned int cycles = bench::measure_avg(4, stdgba_strlen, static_cast<const char*>(str1 + off));
        bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  %d         %u", off, cycles); });
    }

    // strnlen benchmark
    bench::print_header("--- strnlen (len=256) ---");
    fill_string(str1, 256);
    bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  maxlen    cycles"); });
    for (int maxlen : {0, 1, 16, 64, 128, 256, 512}) {
        unsigned int cycles = bench::measure_avg(4, stdgba_strnlen, static_cast<const char*>(str1),
                                                 static_cast<std::size_t>(maxlen));
        bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  %5d    %6u", maxlen, cycles); });
    }
    ...
    for (int len : {0, 1, 4, 8, 16, 32, 64, 128, 256, 1024}) {
        fill_same(len);
        unsigned int cycles = bench::measure_avg(4, stdgba_strcmp, static_cast<const char*>(str1),
    ...
    bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  DiffPos   cycles"); });
    for (int pos : {0, 1, 3, 4, 7, 15, 31, 63}) {
        fill_same(64);
        str2[pos] = str1[pos] + 1;
        unsigned int cycles = bench::measure_avg(4, stdgba_strcmp, static_cast<const char*>(str1),
                                                 static_cast<const char*>(str2));
        bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  %5d    %6u", pos, cycles); });
    }
    ...
    for (int len : {1, 4, 16, 64, 256}) {
        fill_same(len);
        unsigned int cycles = bench::measure_avg(4, stdgba_strncmp, static_cast<const char*>(str1),
    ...
    bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  Size   @pos     cycles"); });
    for (int size : {16, 64, 256, 1024, 4096}) {
        for (int i = 0; i < size; ++i) membuf[i] = 0xAA;
        // Target at end
        membuf[size - 1] = 0x42;
        unsigned int cycles = bench::measure_avg(4, stdgba_memchr, static_cast<const void*>(membuf), 0x42,
                                                 static_cast<std::size_t>(size));
        bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  %4d   end     %6u", size, cycles); });
        membuf[size - 1] = 0xAA;

        // Not found
        unsigned int cycles_nf = bench::measure_avg(4, stdgba_memchr, static_cast<const void*>(membuf), 0x42,
                                                    static_cast<std::size_t>(size));
        bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  %4d   n/a     %6u", size, cycles_nf); });
    }

    // strchr benchmark
    bench::print_header("--- strchr (cycles) ---");
    bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  StrLen @pos     cycles"); });
    for (int len : {16, 64, 256, 1024}) {
        ...
        bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  %4d   end     %6u", len, cycles); });
        str1[len - 1] = 'Z'; // restore
        // Not found
        unsigned int cycles_nf = bench::measure_avg(4, stdgba_strchr, static_cast<const char*>(str1), '~');
        bench::with_logger([&] { bench::log_printf(gba::log::level::info, "  %4d   n/a     %6u", len, cycles_nf); });
    }
