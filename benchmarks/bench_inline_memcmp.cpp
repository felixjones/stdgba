/// @file benchmarks/bench_inline_memcmp.cpp
...
namespace {
    ...
    unsigned int bench_memcmp_bytes_inline() {
        ...
        for (int i = 0; i < ITERS; i++) {
            ...
            total += gba::benchmark::measure([&] {
                ...
                int r = 0;
                if constexpr (N >= 1) {
                    if (!r && pa[0] != pb[0]) r = static_cast<int>(pa[0]) - static_cast<int>(pb[0]);
                }
                if constexpr (N >= 2) {
                    if (!r && pa[1] != pb[1]) r = static_cast<int>(pa[1]) - static_cast<int>(pb[1]);
                }
                if constexpr (N >= 3) {
                    if (!r && pa[2] != pb[2]) r = static_cast<int>(pa[2]) - static_cast<int>(pb[2]);
                }
                if constexpr (N >= 4) {
                    if (!r && pa[3] != pb[3]) r = static_cast<int>(pa[3]) - static_cast<int>(pb[3]);
                }
                if constexpr (N >= 5) {
                    if (!r && pa[4] != pb[4]) r = static_cast<int>(pa[4]) - static_cast<int>(pb[4]);
                }
                if constexpr (N >= 6) {
                    if (!r && pa[5] != pb[5]) r = static_cast<int>(pa[5]) - static_cast<int>(pb[5]);
                }
                if constexpr (N >= 7) {
                    if (!r && pa[6] != pb[6]) r = static_cast<int>(pa[6]) - static_cast<int>(pb[6]);
                }
                if constexpr (N >= 8) {
                    if (!r && pa[7] != pb[7]) r = static_cast<int>(pa[7]) - static_cast<int>(pb[7]);
                }
                result = r;
            });
        }
    ...
    unsigned int bench_memcmp_bytes_call() {
        ...
        for (int i = 0; i < ITERS; i++) {
            asm volatile("" ::: "memory");
            total += gba::benchmark::measure([&] { result = fn_memcmp(a, b, N); });
        }
        return total / ITERS;
    }
            ...
            total += gba::benchmark::measure([&] {
                ...
                for (std::size_t w = 0; w < N / 4; ++w) {
                    ...
                    if (w1 != w2) {
                        ...
                        for (int j = 0; j < 4; ++j) {
                            if (ba[j] != bb[j]) {
                                r = static_cast<int>(ba[j]) - static_cast<int>(bb[j]);
                                break;
                            }
                        }
                        break;
    ...
    unsigned int bench_direct_bcmp() {
        ...
        for (int i = 0; i < ITERS; i++) {
            asm volatile("" ::: "memory");
            total += gba::benchmark::measure([&] { result = fn_bcmp(a, b, N); });
        }
        return total / ITERS;
    }
    ...
    unsigned int bench_wrapper_volatile_n(std::size_t n) {
        ...
        for (int i = 0; i < ITERS; i++) {
            asm volatile("" ::: "memory");
            total += gba::benchmark::measure([&] { result = std::memcmp(a, b, vn); });
        }
        return total / ITERS;
    }
    ...
    unsigned int bench_direct_volatile_n(std::size_t n) {
        ...
        for (int i = 0; i < ITERS; i++) {
            asm volatile("" ::: "memory");
            total += gba::benchmark::measure([&] { result = fn_memcmp(a, b, vn); });
        }
        return total / ITERS;
    }
