...
inline constexpr std::uintptr_t eeprom_bus = 0x0DFFFF00;

inline constexpr std::uint32_t dma_enable    = 1u << 31;
inline constexpr std::uint32_t dma_dst_fixed = 2u << 21;
inline constexpr std::uint32_t dma_src_fixed = 2u << 23;

inline volatile std::uint32_t& dma3sad() noexcept {
    return *reinterpret_cast<volatile std::uint32_t*>(0x040000D4);
}

inline volatile std::uint32_t& dma3dad() noexcept {
    return *reinterpret_cast<volatile std::uint32_t*>(0x040000D8);
}

inline volatile std::uint32_t& dma3cnt() noexcept {
    return *reinterpret_cast<volatile std::uint32_t*>(0x040000DC);
}

/// Send halfwords to EEPROM via DMA3 (src increment, dst fixed).
/// A compiler memory barrier ensures all stores to src are visible
/// before DMA reads from that memory.
inline void dma3_send(const std::uint16_t* src, std::uint32_t count) noexcept {
    dma3cnt() = 0;
    dma3sad() = reinterpret_cast<std::uintptr_t>(src);
    dma3dad() = eeprom_bus;
    asm volatile ("" ::: "memory");
    dma3cnt() = count | dma_enable | dma_dst_fixed;
}

/// Receive halfwords from EEPROM via DMA3 (src fixed, dst increment).
/// A compiler memory barrier after the transfer ensures subsequent reads
/// from dst see the DMA-written values.
inline void dma3_recv(std::uint16_t* dst, std::uint32_t count) noexcept {
    dma3cnt() = 0;
    dma3sad() = eeprom_bus;
    dma3dad() = reinterpret_cast<std::uintptr_t>(dst);
    dma3cnt() = count | dma_enable | dma_src_fixed;
    asm volatile ("" ::: "memory");
}

/// Poll EEPROM until a write operation completes.
inline void poll_ready() noexcept {
    auto* bus = reinterpret_cast<volatile std::uint16_t*>(eeprom_bus);
    while ((*bus & 1) == 0) {}
}
...
block read_block(int address) noexcept {
    ...
    for (int i = AddrBits - 1; i >= 0; --i)
        cmd[idx++] = static_cast<std::uint16_t>((address >> i) & 1);
...
struct basic_istream {
    ...
    /// Seek read position.
    basic_istream& seekg(std::size_t pos) noexcept { m_pos = static_cast<int>(pos); return *this; }
...
struct basic_ostream {
    ...
    /// Seek write position.
    basic_ostream& seekp(std::size_t pos) noexcept { m_pos = static_cast<int>(pos); return *this; }
...
struct basic_iostream {
    ...
    /// Seek read position.
    basic_iostream& seekg(std::size_t pos) noexcept { m_gpos = static_cast<int>(pos); return *this; }

    /// Seek write position.
    basic_iostream& seekp(std::size_t pos) noexcept { m_ppos = static_cast<int>(pos); return *this; }
