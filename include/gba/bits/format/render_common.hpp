...
namespace gba::format::bits {
    ...
    constexpr char grouping_char(format_spec::grouping_kind grouping) {
        switch (grouping) {
            case format_spec::grouping_kind::comma: return ',';
            case format_spec::grouping_kind::underscore: return '_';
            default: return '\0';
        }
    }

    constexpr std::size_t group_size_for(format_spec::format_kind type) {
        switch (type) {
            case format_spec::format_kind::binary:
            case format_spec::format_kind::hex_lower:
            case format_spec::format_kind::hex_upper: return 4;
            default: return 3;
        }
    }

    constexpr bool is_scientific_type(format_spec::format_kind type) {
        return type == format_spec::format_kind::scientific_lower || type == format_spec::format_kind::scientific_upper;
    }

    constexpr bool is_general_type(format_spec::format_kind type) {
        return type == format_spec::format_kind::general_lower || type == format_spec::format_kind::general_upper;
    }

    constexpr bool is_scientific_uppercase(format_spec::format_kind type) {
        return type == format_spec::format_kind::scientific_upper || type == format_spec::format_kind::general_upper;
    }

    constexpr bool uppercase_digits(format_spec::format_kind type) {
        return type == format_spec::format_kind::hex_upper;
    }

    constexpr unsigned int integer_base_for(format_spec::format_kind type) {
        switch (type) {
            case format_spec::format_kind::binary: return 2;
            case format_spec::format_kind::octal: return 8;
            case format_spec::format_kind::hex_lower:
            case format_spec::format_kind::hex_upper: return 16;
            default: return 10;
        }
    }

    constexpr format_spec::align_kind effective_alignment(const format_spec& spec, bool numeric) {
        if (spec.alignment != format_spec::align_kind::none) return spec.alignment;
        if (numeric && spec.zero_pad) return format_spec::align_kind::sign_aware;
        return numeric ? format_spec::align_kind::right : format_spec::align_kind::left;
    }

    constexpr char effective_fill(const format_spec& spec, bool numeric) {
        if (numeric && spec.zero_pad && spec.alignment == format_spec::align_kind::none) return '0';
        return spec.fill;
    }

    constexpr char sign_char_for(bool negative, format_spec::sign_kind sign) {
        if (negative) return '-';
        switch (sign) {
            case format_spec::sign_kind::plus: return '+';
            case format_spec::sign_kind::space: return ' ';
            default: return '\0';
        }
    }
    ...
    constexpr std::size_t write_grouped_digits_impl(UInt value, unsigned int base, bool uppercase,
        ...
        if (grouping == format_spec::grouping_kind::none && type != format_spec::format_kind::grouped) {
            for (std::size_t i = 0; i < count; ++i) out[i] = reversed[count - 1 - i];
            return count;
        }

        const auto groupSize = group_size_for(type);
        const auto separator = grouping == format_spec::grouping_kind::none ? ',' : grouping_char(grouping);
        if constexpr (Runtime) {
