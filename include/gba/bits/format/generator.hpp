...
namespace gba::format {
    ...
    namespace bits {
        ...
        inline constexpr std::uint32_t pow10[] = {1,      10,      100,      1000,      10000,
        ...
        struct hex_emitter {
            ...
            constexpr hex_emitter(std::uint32_t v, bool upper) : value{v}, remaining_nibbles{0}, uppercase{upper} {
    ...
    private:
        ...
        void init_arg_state(const T& value, const format_spec& spec) {
            ...
            if constexpr (std::is_integral_v<T>) {
                ...
                if (spec.fmt_type == format_spec::type_t::hex_lower ||
                    spec.fmt_type == format_spec::type_t::hex_upper) {
                    arg_state.value_kind = arg_stream_state::kind::hex;
                    arg_state.emitter.hex = bits::hex_emitter{static_cast<std::uint32_t>(abs_val),
                                                              spec.fmt_type == format_spec::type_t::hex_upper};
                } else {
        ...
        void init_named_impl(unsigned int hash, const format_spec& spec, std::index_sequence<Is...>) {
            (void)((std::tuple_element_t<Is, decltype(args.args)>::hash == hash
                        ? (init_arg_state(std::get<Is>(args.args).get(), spec), true)
                        : false) ||
                   ...);
        }
    ...
    private:
        ...
        static consteval std::size_t format_arg_static(std::array<char, N>& out, std::size_t pos, unsigned int hash,
                                                       const format_spec& spec, Args... args) {
        ...
        static consteval std::size_t format_arg_static_impl(std::array<char, N>& out, std::size_t pos,
                                                            unsigned int hash, const format_spec& spec, First first,
                                                            Rest... rest) {
            if (First::hash == hash) {
        ...
        static consteval std::size_t format_arg_static_impl(std::array<char, N>& out, std::size_t pos, unsigned int,
                                                            const format_spec&) {
            return pos; // No args - shouldn't happen for valid format
        }

        template<std::size_t N, typename T>
        static consteval std::size_t format_value_static(std::array<char, N>& out, std::size_t pos, const T& value,
