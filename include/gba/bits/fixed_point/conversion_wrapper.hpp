...
// Include operators AFTER defining the concept
...
struct conversion_wrapper_base : conversion_operators<Derived> {
    ...
    constexpr conversion_wrapper_base(const T& value) noexcept
        : m_value(value) {}

    // Implicit conversion back to the wrapped type
    constexpr operator T() const noexcept {
        return m_value;
    }

    // Allow assignment to any fixed<> type using the converter
    template<fixed_point U>
    constexpr operator U() const noexcept {
        return convert_fixed<U>(m_value);
    }

    // Friend declaration for move helper
    template<typename U>
...
constexpr To convert_fixed(const From& from) noexcept {
    ...
    // Shift to adjust fractional bits
    ...
    // Cast to target storage type and reconstruct
