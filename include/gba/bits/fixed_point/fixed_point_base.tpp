// Template implementation for fixed_point_base.hpp
// This file is included at the end of fixed_point_base.hpp

namespace gba {

// Private helper: convert from another fixed-point type (always truncates)
...
constexpr auto fixed<Rep, FracBits, IntermediateRep>::convert_fixed(fixed_point auto rhs) noexcept {
    static constexpr auto shift_right =
        fixed_point_traits<decltype(rhs)>::fractional_digits -
        fixed_point_traits<fixed>::fractional_digits;

    const auto rhsData = __builtin_bit_cast(
        typename fixed_point_traits<decltype(rhs)>::underlying_type,
        rhs
    );
...
// Private helper: check if conversion from Other is trivial
template<std::integral Rep, unsigned int FracBits, std::integral IntermediateRep>
requires (
    FracBits > 0 &&
    FracBits <= std::numeric_limits<std::make_unsigned_t<Rep>>::digits - 1 &&
...
// Floating-point constructor (consteval only, always truncates)
template<std::integral Rep, unsigned int FracBits, std::integral IntermediateRep>
requires (
    FracBits > 0 &&
    FracBits <= std::numeric_limits<std::make_unsigned_t<Rep>>::digits - 1 &&
...
// Implicit conversion from compatible fixed-point types
template<std::integral Rep, unsigned int FracBits, std::integral IntermediateRep>
requires (
    FracBits > 0 &&
    FracBits <= std::numeric_limits<std::make_unsigned_t<Rep>>::digits - 1 &&
...
// Conversion to floating-point (consteval only)
template<std::integral Rep, unsigned int FracBits, std::integral IntermediateRep>
requires (
    FracBits > 0 &&
    FracBits <= std::numeric_limits<std::make_unsigned_t<Rep>>::digits - 1 &&
...
constexpr fixed<Rep, FracBits, IntermediateRep>& fixed<Rep, FracBits, IntermediateRep>::operator+=(fixed rhs) noexcept {
    // Use word-sized types for GBA performance when storage is smaller
...
constexpr fixed<Rep, FracBits, IntermediateRep>& fixed<Rep, FracBits, IntermediateRep>::operator+=(std::integral auto rhs) noexcept {
    // Use word-sized types for GBA performance when storage is smaller
    if constexpr (sizeof(Rep) < sizeof(int)) {
        using WordType = std::conditional_t<std::is_signed_v<Rep>, int, unsigned int>;
        const auto lhs_word = static_cast<WordType>(m_data);
...
// Binary arithmetic operators for same type
template<fixed_point T>
...
// Binary arithmetic operators with integers
template<fixed_point T>
