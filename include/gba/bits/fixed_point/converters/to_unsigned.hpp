...
struct unsigned_wrapper : conversion_wrapper_base<T, unsigned_wrapper<T>> {
    ...
    static constexpr auto convert_binary_result(const L& lhs, const R& rhs, Op op) noexcept {
        ...
        // Convert LHS to unsigned (same size)
        using lhs_unsigned = std::make_unsigned_t<lhs_rep>;

        // Result representation: use larger of unsigned LHS or RHS
        using result_rep = std::conditional_t<
            (sizeof(lhs_unsigned) > sizeof(rhs_rep)), lhs_unsigned,
            std::conditional_t<
                std::is_unsigned_v<rhs_rep>, rhs_rep, lhs_unsigned
            >
        >;
