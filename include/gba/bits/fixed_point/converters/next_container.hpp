...
namespace gba {
    ...
    struct next_container_wrapper : conversion_wrapper_base<T, next_container_wrapper<T>> {
        ...
        struct next_size {
            using type = std::conditional_t<
                ...
                std::conditional_t<sizeof(Rep) >= sizeof(int), long long,
                                   std::conditional_t<sizeof(Rep) >= sizeof(short), int,
                                                      std::conditional_t<sizeof(Rep) >= sizeof(char), short, short>>>>;
        };
        ...
        static constexpr auto convert_binary_result(const L& lhs, const R& rhs, Op op) noexcept {
            ...
            using result_rep = std::conditional_t<std::is_signed_v<lhs_rep> || std::is_signed_v<rhs_rep>,
