...
namespace gba {
    ...
    struct signed_wrapper : conversion_wrapper_base<T, signed_wrapper<T>> {
        ...
        struct make_signed_safe {
            using type =
                std::conditional_t<std::is_signed_v<Rep>, Rep,
                                   std::conditional_t<sizeof(Rep) >= sizeof(int), long long,
                                                      std::conditional_t<sizeof(Rep) >= sizeof(short), int, short>>>;
        };
        ...
        static constexpr auto convert_binary_result(const L& lhs, const R& rhs, Op op) noexcept {
            ...
            using result_rep = std::conditional_t<(sizeof(lhs_signed) > sizeof(rhs_rep)), lhs_signed,
                                                  std::conditional_t<std::is_signed_v<rhs_rep>, rhs_rep, lhs_signed>>;
