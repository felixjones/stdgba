...
fixed(U) -> fixed<
    typename fixed_point_traits<U>::underlying_type,
    fixed_point_traits<U>::fractional_digits
>;
...
// Bit-cast helpers for fixed-point types
// - bit_cast(x) returns the inner representation
// - bit_cast<Rep>(x) returns a requested integer type
...
// Make fixed_point types arithmetic for std
namespace std {
