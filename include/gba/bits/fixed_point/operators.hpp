...
struct conversion_operators {
    ...
    constexpr auto operator+() const noexcept {
        return +move(static_cast<const Derived&>(*this));
    }
    ...
    // Comparison operators - delegate to wrapped value
...
// Helper to unwrap conversion wrappers or return the value as-is
template<typename T>
...
// Wrapper + Wrapper (only allowed if same wrapper type)
...
constexpr auto operator+(const L& lhs, const R& rhs) noexcept {
    return L::convert_binary_result(unwrap(lhs), unwrap(rhs),
...
// Wrapper + fixed_point (non-wrapper)
...
constexpr auto operator+(const W& lhs, const F& rhs) noexcept {
    return W::convert_binary_result(unwrap(lhs), rhs,
        [](const auto& a, const auto& b) { return a + b; });
}
...
constexpr auto operator+(const F& lhs, const W& rhs) noexcept {
    return W::convert_binary_result(lhs, unwrap(rhs),
        [](const auto& a, const auto& b) { return a + b; });
}
