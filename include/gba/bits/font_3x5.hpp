...
namespace gba::bits {
    ...
    inline constexpr std::array<std::uint16_t, 96> font_3x5_data = {
    ...
    constexpr unsigned get_font_line(std::uint16_t glyph, int lineNum) {
        const unsigned b0 = static_cast<unsigned>(glyph) >> 8u;
        const unsigned b1 = static_cast<unsigned>(glyph) & 0xFFu;
        if ((b1 & 1u) != 0u) lineNum -= 1;
