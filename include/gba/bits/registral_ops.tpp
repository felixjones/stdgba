...
namespace gba::bits {
    ...
    inline typename Self::value_type read_ops::value(this const Self& self) noexcept {
        ...
        if constexpr (sizeof(value_type) == 1) {
            ...
            asm volatile(".set i, 0\n"
                         ...
                         ".endr"
                         : [temp] "=&l"(temp)
                         : [address] "l"(self.m_address), [destination] "l"(destination), "i"(sizeof(value_type))
                         : "memory");
            return __builtin_bit_cast(value_type, destination);
        } else {
    ...
    [[gnu::always_inline]]
    inline void write_by_copy6(std::uintptr_t address, auto value)
        requires(sizeof(decltype(value)) == 6)
    {
        auto valuePtr = &value;
        asm volatile("ldm %[value]!, {r1-r2}\n"
                     "str r1, [%[address]]\n"
                     "strh r2, [%[address], #4]"
                     : [value] "+l"(valuePtr)
                     : [address] "l"(address)
    ...
    inline void write_ops::write_integer(this const Self& lhs, std::integral auto value) noexcept {
        ...
        if constexpr (sizeof(value) > sizeof(value_type)) {
            if constexpr (sizeof(value_type) == 1) {
                asm volatile("strb %[value], [%[address]]" ::[value] "l"(value), [address] "l"(lhs.m_address)
                             : "memory");
            } else if constexpr (sizeof(value_type) == 2) {
                asm volatile("strh %[value], [%[address]]" ::[value] "l"(value), [address] "l"(lhs.m_address)
                             : "memory");
            } else if constexpr (sizeof(value_type) == 4) {
                asm volatile("str %[value], [%[address]]" ::[value] "l"(value), [address] "l"(lhs.m_address)
                             : "memory");
            } else {
    ...
    [[gnu::always_inline]]
    inline void write_ops::copy(this const Self& lhs, const Other& rhs) noexcept
