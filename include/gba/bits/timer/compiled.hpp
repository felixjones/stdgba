...
namespace gba {
    ...
    struct compiled_timer {
        ...
        [[nodiscard]]
        constexpr const timer_config& operator[](std::size_t i) const noexcept {
            return m_timers[i];
        }

        /// @brief Iterator to the first timer configuration.
        [[nodiscard]]
        constexpr const timer_config* begin() const noexcept {
            return m_timers.data();
        }

        /// @brief Iterator past the last active timer configuration.
        [[nodiscard]]
        constexpr const timer_config* end() const noexcept {
            return m_timers.data() + m_count;
        }

        /// @brief Pointer to the underlying timer configuration data.
        [[nodiscard]]
        constexpr const timer_config* data() const noexcept {
            return m_timers.data();
        }
    };
    ...
    namespace bits {
        ...
        consteval compiled_timer build_timer(long long total, gba::cycles prescaler, bool irq,
            ...
            if (total < 0x10000) {
                ...
                result.m_timers[0] = timer_config{
                    static_cast<unsigned short>(-total), {.cycles = prescaler, .overflow_irq = irq, .enabled = true}
                };
                return result;
            }
            ...
            if (cascade1 < 0x10000) {
                result.m_count = 2;
                result.m_timers[0] = timer_config{
                    static_cast<unsigned short>(-base), {.cycles = prescaler, .enabled = true}
                };
                result.m_timers[1] = timer_config{
                    static_cast<unsigned short>(-cascade1), {.cascade = true, .overflow_irq = irq, .enabled = true}
                };
                return result;
            }
            ...
            if (cascade2 < 0x10000) {
                ...
                result.m_timers[1] = timer_config{
                    static_cast<unsigned short>(-cascade1), {.cascade = true, .enabled = true}
                };
                result.m_timers[2] = timer_config{
                    static_cast<unsigned short>(-cascade2), {.cascade = true, .overflow_irq = irq, .enabled = true}
                };
                return result;
            }
            ...
            if (cascade3 < 0x10000) {
                ...
                result.m_timers[2] = timer_config{
                    static_cast<unsigned short>(-cascade2), {.cascade = true, .enabled = true}
                };
                result.m_timers[3] = timer_config{
                    static_cast<unsigned short>(-cascade3), {.cascade = true, .overflow_irq = irq, .enabled = true}
                };
                return result;
            }
        ...
        consteval compiled_timer compile_timer_impl(std::chrono::duration<Rep, Period> duration, bool irq,
                                                    std::size_t extraTimers) {
        ...
        template<typename Rep, typename Period>
        consteval compiled_timer compile_timer_exact_impl(std::chrono::duration<Rep, Period> duration, bool irq,
                                                          std::size_t extraTimers) {
            using duration_1024 = std::chrono::duration<long long, std::ratio_divide<clock_ratio, std::ratio<1, 1024>>>;
            using duration_256 = std::chrono::duration<long long, std::ratio_divide<clock_ratio, std::ratio<1, 256>>>;
            using duration_64 = std::chrono::duration<long long, std::ratio_divide<clock_ratio, std::ratio<1, 64>>>;
            ...
            if (const auto c = std::chrono::duration_cast<duration_1024>(duration);
                std::chrono::duration_cast<dur>(c).count() == static_cast<long long>(duration.count()) &&
                c.count() > 0) {
        ...
        template<typename Rep, typename Period>
        consteval compiled_timer compile_cycle_timer_impl(std::chrono::duration<Rep, Period> cycles, bool irq,
                                                          std::size_t cascadeTimers) {
            static_assert(std::ratio_equal_v<Period, clock_ratio> ||
            ...
            constexpr auto prescaler =
                ...
                : std::ratio_equal_v<Period, std::ratio_divide<clock_ratio, std::ratio<1, 256>>> ? cycles_256
                                                                                                 : cycles_1024;
    ...
    /// @endcode
    template<typename Rep, typename Period>
    consteval compiled_timer compile_timer(std::chrono::duration<Rep, Period> duration, bool irq = false,
                                           std::size_t extraTimers = 3) {
        return bits::compile_timer_impl(duration, irq, extraTimers);
    }
    ...
    /// @endcode
    template<typename Rep, typename Period>
    consteval compiled_timer compile_timer_exact(std::chrono::duration<Rep, Period> duration, bool irq = false,
                                                 std::size_t extraTimers = 3) {
        return bits::compile_timer_exact_impl(duration, irq, extraTimers);
    }
    ...
    /// @endcode
    template<typename Rep, typename Period>
    consteval compiled_timer compile_cycle_timer(std::chrono::duration<Rep, Period> cycles, bool irq = false,
                                                 std::size_t cascadeTimers = 3) {
        return bits::compile_cycle_timer_impl(cycles, irq, cascadeTimers);
    }
