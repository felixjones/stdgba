...
consteval std::pair<long double, long double> sin_cos(unsigned int alpha16) {
    ...
    // Reduce alpha to 0..65535
    alpha16 &= 0xFFFFu;

    // Quadrant = top two bits of alpha16 (each quadrant = 16384 units)
    ...
    switch (quadrant) {
        case 0: sin_res = s; cos_res = c; break;
        case 1: sin_res = c; cos_res = -s; break;
        case 2: sin_res = -s; cos_res = -c; break;
        default: sin_res = -c; cos_res = s; break; // quadrant 3
    }
