...
consteval cmd write_page(int page, write_fn fn) {
    if (page < 0 || page >= pages_per_bank)
        throw "write_page: page index out of range (0-511)";
    if (fn == nullptr)
        throw "write_page: write function must not be null";
...
consteval cmd read_page(int page, read_fn fn) {
    if (page < 0 || page >= pages_per_bank)
        throw "read_page: page index out of range (0-511)";
    if (fn == nullptr)
        throw "read_page: read function must not be null";
        ...
        for (std::size_t i = 0; i < count; ++i) {
            ...
            switch (c.kind) {
                case cmd::write_page_k:
                    ...
                    if (flash::detail::write_atmel_page(addr, buffer.data()) != 0)
                        return error::timeout;
                    break;

                case cmd::read_page_k:
                    flash::detail::read_bytes(buffer.data(), flash::detail::flash_ptr(addr), page_size_atmel);
