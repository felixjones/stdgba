        ...
        consteval bool validate(const cmd* commands, std::size_t count) {
            ...
            for (std::size_t i = 0; i < count; ++i) {
                ...
                switch (c.kind) {
                    ...
                    case cmd::erase_chip_k: chip_erased = true; break;

                    case cmd::write_sector_k:
                        if (chip_erased) break;
                        if (is_arg) {
                            if (!arg_erased[c.arg_index]) return false;
                        } else if (!bank_is_arg) {
                            if (!erased[bank][c.index]) return false;
                        } else {
                            return false;
                        }
                        break;
    ...
    private:
        error execute_impl(const int* rt) const noexcept {
            ...
            for (std::size_t i = 0; i < count; ++i) {
                ...
                switch (c.kind) {
                    case cmd::erase_sector_k:
                        if (flash::bits::erase_sector(base) != 0) return error::timeout;
                        break;

                    case cmd::erase_chip_k:
                        if (flash::bits::erase_chip() != 0) return error::timeout;
                        break;

                    case cmd::write_sector_k:
                        c.writer(sector_span{as_byte(), sector_size});
                        for (std::size_t j = 0; j < sector_size; ++j) {
                            if (flash::bits::write_byte(base + j, buffer[j]) != 0) return error::timeout;
                        }
                        break;

                    case cmd::read_sector_k:
                        flash::bits::read_bytes(buffer.data(), flash::bits::flash_ptr(base), sector_size);
