...
namespace gba::bits {
    ...
    template<std::integral T, std::ranges::input_range R>
    ...
    struct integer_view : std::ranges::view_interface<integer_view<T, R>> {
        ...
        public:
            ...
            explicit constexpr iterator(R& base) noexcept
                : m_elementIterator(std::ranges::begin(base)), m_endIterator{std::ranges::end(base)} {}
    ...
    constexpr auto min_max(const View& r) noexcept
        requires std::integral<std::ranges::range_value_t<View>>
    {
    ...
    consteval auto ranges_frequency(std::ranges::input_range auto&& r) {
        ...
        for (const auto& value : r) {
            auto it = std::find_if(frequencies.begin(), frequencies.begin() + uniqueCount,
        ...
        std::ranges::sort(frequencies.begin(), frequencies.begin() + uniqueCount, [](const auto& a, const auto& b) {
